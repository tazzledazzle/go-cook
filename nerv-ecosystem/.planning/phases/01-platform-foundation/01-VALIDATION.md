...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 01-01-01 | 01-01 | 1 | PLAT-03 | T-1-SC | pinned deps + `go mod verify`; RED gate before any production code (this task **is** Wave 0) | setup + RED | `go mod verify && { go test ./... -race > /tmp/01-01-red.log 2>&1; test $? -ne 0; } && grep -qE "undefined: (store\|cmd)\." /tmp/01-01-red.log` | ❌ created here | ⬜ pending |
| 01-01-02 | 01-01 | 1 | PLAT-02 | T-1-01, T-1-02, T-1-03, T-1-04 | parameterized SQL only; store dir `0o700`; WAL pragma read back rather than assumed; `SetMaxOpenConns(1)` | unit | `go test ./internal/store/... -race -count=1` | ❌ W0 = 01-01-01 | ⬜ pending |
| 01-01-03 | 01-01 | 1 | PLAT-01 | T-1-03 | no network/daemon; `cmd/` imports no `database/sql`; store failure returns a wrapped error, not a panic | integration | `go test ./... -race -count=1 && go build -o /tmp/modular-01-01 . && MODULAR_HOME=$(mktemp -d) /tmp/modular-01-01 status \| grep -q "journal_mode: wal"` | ❌ W0 = 01-01-01 | ⬜ pending |
...
- [ ] `go.mod` / `go.sum` with testify v1.11.1, modernc.org/sqlite v1.54.0, cobra v1.10.2, go-cmp v0.7.0
- [ ] `internal/store/store_test.go` — PLAT-02 (WAL mode + FTS5 table existence)
- [ ] `cmd/status_test.go` — PLAT-01 (Cobra `SetArgs`/`SetOut`/`Execute` against a temp store)
